...
namespace MacroScanAI
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            // The first TabItem:
            TabItem firstTab = MyTabControl.Items[0] as TabItem;

            // The FileSession inside it:
            if (InitialFileSession != null)
            {
                InitialFileSession.FileOpened += (fileName) =>
                {
                    string fileNameOnly = System.IO.Path.GetFileName(fileName);
                    firstTab.Header = fileNameOnly;
                };
            }
        }
        ...
        private void AddNewTab()
        {
            ...
            FileSession aFileSession = new FileSession();

            newTab.Header = "Untitled" + MyTabControl.Items.Count;

            // Hook into FileSession's event so we can update the tab header
            aFileSession.FileOpened += (fileName) =>
            {
                string fileNameOnly = System.IO.Path.GetFileName(fileName);
                newTab.Header = fileNameOnly;
            };

            newTab.Content = aFileSession;
